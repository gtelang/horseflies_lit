...
def animate_tour (sites, inithorseposn, phi, horse_trajectory, fly_trajectories, animation_file_name_prefix):
    ...
    ax.set_xlabel(r"$\varphi$ = " + str(phi) + "  Number of drones is " + str(number_of_flies), 
                  fontsize=20)
    ...
    ims               = []
    fly_sites_so_far  = [[] for i in range(number_of_flies)] # list of sites for each fly that have been covered so far
        ...
        for k in range(len(horse_leg_disc['points'])):
            ...
            for j in range(len(fly_legs_disc)):
                  if fly_legs_disc[j] != None:
                        print "********************"
                        print Fore.RED, "Type of leg: ", fly_legs_disc[j]['legtype']
                        print Fore.GREEN
                        utils_algo.print_list(fly_legs_disc[j]['points'])
                        print Style.RESET_ALL
                        current_fly_posns.append(fly_legs_disc[j]['points'][k])
                        # update the fly_points_so_far thing here depending on legtype
                        print Fore.RED, j, " ", k, Style.RESET_ALL
                        if fly_legs_disc[j]['legtype'] == 'gsg' and k==9:
                              fly_sites_so_far[j].append(fly_legs_disc[j]['points'][k])
                  else: 
                        current_fly_posns.append(None)
            print "................................................."
            print "Current horse position is", (current_horse_posn)
            print "Current positions of flies are "
            utils_algo.print_list(current_fly_posns)
    ...
    print "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
    utils_algo.print_list(fly_sites_so_far)
    print "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
