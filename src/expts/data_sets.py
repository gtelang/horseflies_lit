...
def annulus(N):
    """ Uniform on a circle placed regularly and then perturbed
    """
    shift = np.asarray([0.5,0.5])
    xs = [ 0.70 * np.cos(2*i*np.pi/N) * 0.5  + shift[0] + random.choice([-1,1])* np.random.rand()/20.0 for i in range(N)]
    ys = [ 0.70 * np.sin(2*i*np.pi/N) * 0.5  + shift[1] + random.choice([-1,1])* np.random.rand()/20.0 for i in range(N)]
    return map(np.asarray, zip(xs,ys))
...
def genpointset(numpts, cloudtype):
    ...
    if cloudtype == 'uni':
    ...
    elif cloudtype == 'clusunif':
        return clusunif(numpts)
        ...
        print "Key for cloud type not recognized "
        sys.exit()
