...
\mbox{}\verb@     @\hbox{$\langle\,${\itshape Parse \verb|horse_trajectory| and \verb|fly_trajectories| and set up required data-structures}\nobreak\ {\footnotesize \NWlink{nuweb85a}{85a}}$\,\rangle$}\verb@@\\
...
\NWtarget{nuweb85a}{} $\langle\,${\itshape Parse \verb|horse_trajectory| and \verb|fly_trajectories| and set up required data-structures}\nobreak\ {\footnotesize {85a}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@ax.set_title("Number of sites: " + str(len(sites)), fontsize=25)@\\
\mbox{}\verb@ax.set_xlabel(r"$\varphi$ = " + str(phi), fontsize=20)@\\
...
\mbox{}\verb@yhs = [ horse_trajectory[i][0][1] for i in range(len(horse_trajectory))]    @\\
\mbox{}\verb@@\\
\mbox{}\verb@# x and y coordinates of each recorded vertex on every fly trajectory@\\
\mbox{}\verb@xfss = [[point['coordinates'][0] for point in fly_trajectories[i]] for i in range(len(fly_trajectories))]@\\
\mbox{}\verb@yfss = [[point['coordinates'][1] for point in fly_trajectories[i]] for i in range(len(fly_trajectories))]@\\
\mbox{}\verb@@{\NWsep}
\end{list}
...
A recorded point on a fly's trajectory is marked as either of type ``genpt''(\verb|g|) or ``site''(\verb|s|). 
Then a fly-leg is either a sequence of points of type ``\verb|gg|'' or ``\verb|gsg|''.  

A horse-leg is, as before, is  a tuple of two successive points along its trajectory. 

For both the horse and flies, their trajectories consist of their corresponding legs joined end-to-end. An important
invariant for constructing the animation is that every fly's trajectory has the same number of legs as the
horse's trajectory.
...
\mbox{}\verb@ims = []@\\
\mbox{}\verb@for horse_leg, leg_idx in zip(horse_legs, range(len(horse_legs))):@\\
\mbox{}\verb@     debug(Fore.YELLOW + "Animating leg: "+ str(leg_idx) + Style.RESET_ALL)@\\
\mbox{}\verb@@{\NWsep}
\end{list}
\vspace{-1.5ex}
...
\NWtarget{nuweb85c}{} $\langle\,${\itshape Extract the list of horse and fly legs along the horse and fly trajectories respectively}\nobreak\ {\footnotesize {85c}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@   @\\
\mbox{}\verb@@{\NWsep}
...
\vspace{-0.8cm} \newchunk
%{python-mode}%
\begin{flushleft} \small\label{scrap139}\raggedright\small
\NWtarget{nuweb85d}{} $\langle\,${\itshape Write animation of tour to disk and display in live window}\nobreak\ {\footnotesize {85d}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@from colorama import Back @\\
\mbox{}\verb@@\\
...
\item $\langle\,$Parse \verb|horse_trajectory| and \verb|fly_trajectories| and set up required data-structures\nobreak\ {\footnotesize \NWlink{nuweb85a}{85a}}$\,\rangle$ {\footnotesize {\NWtxtRefIn} \NWlink{nuweb84a}{84a}.}
...
\item $\langle\,$Write animation of tour to disk and display in live window\nobreak\ {\footnotesize \NWlink{nuweb85d}{85d}}$\,\rangle$ {\footnotesize {\NWtxtRefIn} \NWlink{nuweb84a}{84a}.}
